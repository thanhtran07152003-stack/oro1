...
const MAX_RETRIES = 3;
const TIMEOUT_MS = 8000; // Tăng timeout lên 8 giây
const RETRY_DELAY_MS = 3000; // Tăng delay thử lại lên 3 giây
const RATE_LIMIT_DELAY_MS = 15000; // Tăng delay lỗi 429 lên 10 giây
const TRANSACTION_DELAY_MS = 25000; // Tăng delay giữa các giao dịch lên 15 giây
...
async function checkProxyIP(proxy, userAgents) {
    for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
        try {
            ...
            log(`Thử lần ${attempt}/${MAX_RETRIES} thất bại khi kiểm tra proxy ${proxy}: ${error.message} (Code: ${error.code || error.name}, Error Code: ${error.error?.code || 'N/A'})`, 'error');
            if ((error instanceof TimeoutError || error.code === 'ECONNABORTED' || error.code === -32008) && attempt < MAX_RETRIES) {
                log(`${error instanceof TimeoutError ? 'Timeout' : 'Lỗi mạng'}, thử lại sau ${RETRY_DELAY_MS / 1000} giây...`, 'warning');
...
const validProxiesCache = new Map(); // Lưu trữ proxy đã kiểm tra
...
async function getValidProxy(proxies, userAgents, walletIndex) {
    ...
    while (availableProxies.length > 0) {
        ...
        if (validProxiesCache.has(proxy)) {
            log(`[Ví ${walletIndex}] ✅ Sử dụng proxy đã kiểm tra từ cache: ${proxy} (IP: ${validProxiesCache.get(proxy)})`);
            return proxy;
        }

        try {
            const ip = await checkProxyIP(proxy, userAgents);
            validProxiesCache.set(proxy, ip); // Lưu vào cache
    ...
    log(`[Ví ${walletIndex}] ❌ Không tìm được proxy hoạt động`, 'error');
    return null;
}
...
async function runBotForWallet(mnemonic, walletIndex, proxyUrl, userAgents, proxies) {
    ...
    for (let liqCount = 0; liqCount < 1; liqCount++) {
        ...
        for (let i = 0; i < 10; i++) {
            await swap(mnemonic, ZIG_AMOUNT, CONFIG.zigDenom, CONFIG.oroDenom, proxyUrl, walletIndex, userAgents, proxies);
            await delay(TRANSACTION_DELAY_MS, walletIndex);
        }

        for (let i = 0; i < 10; i++) {
            await swap(mnemonic, ORO_AMOUNT, CONFIG.oroDenom, CONFIG.zigDenom, proxyUrl, walletIndex, userAgents, proxies);
            await delay(TRANSACTION_DELAY_MS, walletIndex);
        }
        ...
        for (let i = 0; i < 5; i++) {
            const poolInfo = await getPoolRatio(proxyUrl, walletIndex, userAgents, proxies);
            if (poolInfo) {
                await addLiquidity(mnemonic, LIQ_ORO, proxyUrl, walletIndex, userAgents, proxies);
                await delay(TRANSACTION_DELAY_MS, walletIndex);
            } else {
...
async function runMultiWalletBotParallel() {
    const mnemonics = loadFile("phrase.txt");
    ...
    const THREADS = Math.min(5, mnemonics.length); // Giảm xuống 5 luồng
    for (let i = 0; i < mnemonics.length; i += THREADS) {
        const batch = mnemonics.slice(i, i + THREADS);
        log(`\n🔄 Xử lý lô ví ${i + 1} đến ${Math.min(i + THREADS, mnemonics.length)}...`);

        const tasks = batch.map(async (mnemonic, index) => {
            const walletIndex = i + index + 1;
            if (await validateMnemonic(mnemonic)) {
                ...
                if (proxy) {
                    ...
                    await runBotForWallet(mnemonic, walletIndex, proxy, userAgents, proxies);
                } else {
